    ...
    if length == 0:
        ...
        for i in r:
            for k, v in i.items():
                if k == 'id':
                    id_list.append(v)
        print(f"[INFO] получено {length} писем")

        cur_dir = os.getcwd()
        final_dir = os.path.join(cur_dir, 'all_mails')
        ...
        for i in id_list:
            ...
            with open(mail_file_path, 'w', encoding='utf8') as file:
...
def delete_mail(mail):
    ...
    print(f"[INFO] Почта {mail} успешно удалена!")
...
def main():
    try:
        ...
        print(f"Почта : {mail}  готова к использованию!")
